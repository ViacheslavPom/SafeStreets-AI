...
const ensureHeatSource = (map) => {
  ...
  const pts = Array.from({length: 200}, ()=>{
    const dx = (Math.random()-0.5)*0.2, dy = (Math.random()-0.5)*0.2
    return { type:'Feature', properties:{ mag: Math.random() }, geometry:{ type:'Point', coordinates:[-73.985130+dx, 40.758896+dy] } }
  })
  map.addSource(HEAT_SOURCE_ID, { type:'geojson', data:{ type:'FeatureCollection', features: pts } })
}
...
const upsertHeatLayer = (map, visible) => {
  ...
  if (!map.getLayer(HEAT_LAYER_ID)) {
    ...
    Object.entries(HEAT_PAINT).forEach(([k,v])=>{
      try { map.setPaintProperty(HEAT_LAYER_ID, k, v) } catch {}
    })
  ...
  if (firstSymbol) {
    try { map.moveLayer(HEAT_LAYER_ID, firstSymbol.id) } catch {}
  }
}
...
function applyHackopolyTheme(map) {
  try {
    ...
    const patchTheme = () => {
      ...
      try {
        map.setFog(null)
        map.setFog({ range:[0.1,1], color:'#ffffff', horizonBlend:0.05, highColor:'#f0f8ff', spaceColor:'#f0f8ff', starIntensity:0 })
      } catch(_){}
...
`;
  ...
  useEffect(() => {
    ...
    const onDrawRoutes = (e) => {
      ...
      const upd = (n,data)=>{ try{ const s = m.getSource(n+'-src'); if(s && data) s.setData(data); }catch(_){} }
      upd('fastest', fastest); upd('safest', safest); upd('weighted', weighted);
      ...
      const getLineEnd = (lines)=>{
        for(let i=lines.length-1;i>=0;i--){
          const line = lines[i];
          if(line && line.length){ return line[line.length-1]; }
        }
        return null;
      }
      ...
      if(lines.length){
        ...
        if(b){ try{ m.fitBounds(b, { padding: 60, maxZoom: 16, duration: 500 }); }catch(_){} }
    ...
    const onZoomIn  = () => { try { const m = map(); if(!m) return; m.zoomTo(Math.min(m.getZoom()+1, 20), { duration: 250 }) } catch(_){} }
    const onZoomOut = () => { try { const m = map(); if(!m) return; m.zoomTo(Math.max(m.getZoom()-1, 1),  { duration: 250 }) } catch(_){} }
    const onGeolocate = () => { try { const m = map(); if(!m) return; const center = window.__SS_HARDCODED_CENTER || [-73.985130, 40.758896]; m.flyTo({ center, zoom: Math.max(m.getZoom(), 14), essential: true }) } catch(_){} }
